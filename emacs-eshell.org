    ...
    PWD is not in a git repo (or the git command is not found)."
      ...
      (when (and (not (file-remote-p pwd))
        ...
        (let* ((git-url (shell-command-to-string "git config --get remote.origin.url"))
               ...
               (git-icon  "\xe0a0")
               (git-icon2 (propertize "\xf020" 'face `(:family "-*-github-octicons-normal-normal-normal-*-*-*-*-*-p-0-iso10646-1")))
               (git-icon3 (all-the-icons-octicon "git-branch")))
  ...
  #+BEGIN_SRC elisp
    ...
    has been called."
      (when (fboundp #'pyenv-mode-version)
        (if-let ((venv (pyenv-mode-version)))
