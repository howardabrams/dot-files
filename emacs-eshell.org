    ...
    (setq eshell-prompt-function
          (lambda ()
            (let* ((directory (split-directory-prompt (pwd-shorten-dirs (pwd-replace-home (eshell/pwd)))))
                   ...
                   (ruby (string-trim (shell-command-to-string "~/.rvm/bin/rvm-prompt")))
                   (python (concat "Py:" (pyenv-mode-version))))

              (if (eq 'dark (frame-parameter nil 'background-mode))
                  (concat   ;; Prompt for Dark Themes
                   (propertize "┌─ " 'face `(:foreground "green"))
                   (propertize parent 'face `(:foreground "#8888FF"))
                   (propertize name   'face `(:foreground "#aaaaFF" :weight bold))
                   (propertize " ──" 'face `(:foreground "green"))
                   (propertize branch 'face `(:foreground "yellow"))
                   (propertize " ── " 'face `(:foreground "green"))
                   (propertize ruby 'face `(:foreground "red"))
                   (propertize " ── " 'face `(:foreground "green"))
                   (propertize python 'face `(:foreground "#5555FF"))
