...
*** EWW
    ...
    For v24.4, I like using EWW...especially if I can customize and
    trim the resulting text. Something to work on, eh?

    #+BEGIN_SRC
    (when (fboundp 'eww)
      (global-set-key (kbd "<f9> b") 'eww)
      (global-set-key (kbd "<f9> l") 'eww-follow-link)
      (global-set-key (kbd "<f9> u") 'eww-follow-url))
    #+END_SRC
...
#+END_SRC
    ...
    This works really well with other commands, including [[https://github.com/Bruce-Connor/fancy-narrow][fancy-narrow]]:
    ...
    #+BEGIN_SRC elisp
      (when (autofeaturep 'fancy-narrow)
        ...
        (defun ha-highlight-section ()
        "If If the region is active, call 'fancy-narrow-to-region on it, otherwise, call 'fancy-narrow-to-defun, and see what happens."
        (interactive)
...
** Editing Root Files
   ...
   The trick, as always, is finding the correct keybinding:

   #+BEGIN_SRC elisp
   (global-set-key (kbd "C-x C-S-f") 'ha/find-file-as-root)
   #+END_SRC
