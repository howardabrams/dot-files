...
*** Tag Support
   ...
   Notice that I don't set Helm for everything.
...
*** Code Block Folding
    ...
    The [[info:emacs#Hideshow][Hide Show Minor]] mode allows us to load a programming file with
    all functions folded, showing only the header lines:

    #+BEGIN_SRC elisp
      (defun ha/after-load (file)
        "Called after every file is loaded."
        (when (derived-mode-p 'prog-mode)
          (hs-minor-mode 1)
          (hs-hide-all)))

      (add-to-list 'after-load-functions 'ha/after-load)
    #+END_SRC

    See the [[http://www.emacswiki.org/emacs/HideShow][online resources]].

    Note: =C-c @ C-c= toggles the showing or hiding of a block. Little
    quicker:
