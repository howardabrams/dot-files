...
** Grep for my Notes
   ...
   Best part about the ag package, is not needing to do any
   configuration (as it is all load-on demand).

   - =ag-project= is the primary interface for me
   - =ag-project-at-point= sets the query with the word at point

   The only downside is needing to pre-define [[file:~/.agignore][ignorable files]]:
