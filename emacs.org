...
#+BEGIN_SRC elisp
  ...
  (global-set-key (kbd "C-x C-n") 'locate-org-filesq)
#+END_SRC

   The =mdfind= command is pretty powerful and has quite the scripting
   language, so I might toy with the following:

#+BEGIN_SRC elisp :tangle no
  (defun spot-locate-make-command-line (search-string)
    (list "mdfind" "-interpret" search-string))
  (setq locate-make-command-line 'spot-locate-make-command-line)
#+END_SRC
